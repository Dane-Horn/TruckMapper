  ...
  Papa.parse(file, {
    ...
    complete: results => {
      max = results.data.reduce((prev, curr) => {
        if (curr["Veloctiy (km/h)"] > prev) return curr["Veloctiy (km/h)"];
        return prev;
      }, 0);
      ...
      $("#status").html(
        `First reading: ${
          results.data[0]["Time of reading (South Africa Standard Time)"]
        } <br> Last reading: ${
          results.data.pop()["Time of reading (South Africa Standard Time)"]
        }`
      );
